...
export default function Practice() {
  ...
  useEffect(() => {
    if (location.state?.speechId) {
      ...
      fetchPreviousScore(location.state.speechId);
    }
    
    if (location.state?.preloadedSpeech) {
      setReferenceText(location.state.preloadedSpeech);
    }
  }, [location]);

  const fetchPreviousScore = async (speechId) => {
    try {
      const token = localStorage.getItem('token');
      ...
      if (data.success && data.sessions) {
        const speechSessions = data.sessions.filter(s => 
          s.speechId && s.speechId._id === speechId
        );
        
        if (speechSessions.length > 0) {
          setPreviousScore(speechSessions[0].score);
        }
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 relative overflow-hidden">
      ...
      <div className="relative z-10 px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          {referenceText && (
            <div className="bg-gradient-to-br from-indigo-900/50 to-purple-900/50 backdrop-blur-sm rounded-3xl shadow-2xl p-8 mb-6 border border-white/10">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-2xl font-black text-white flex items-center gap-2">
                  <svg className="w-7 h-7 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  📖 Your Speech Reference
                </h3>
                <button
              ...
              <p className="text-purple-200 text-sm mt-3 flex items-center gap-2">
                ...
                Read this while recording to stay on track
              </p>
            </div>
          )}
